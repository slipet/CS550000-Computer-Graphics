...
GLint cur_mag = GL_NEAREST;
GLint cur_min = GL_LINEAR;
GLint cur_coord = GL_REPEAT;
...
//vector<string> model_list{ "../TextureModels/Fushigidane.obj", "../TextureModels/Mew.obj","../TextureModels/Nyarth.obj","../TextureModels/Zenigame.obj" };
GLuint program;
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	glUniform1i(iLocShader, per_vertex_or_per_pixel);


	//cout << models[cur_idx].shapes[0].material.offsets.size() << endl;
	...
	for (int i = 0; i < models[cur_idx].shapes.size(); i++) 
	{
		...
		glActiveTexture(models[cur_idx].shapes[i].material.diffuseTexture);

		glBindTexture(GL_TEXTURE_2D, models[cur_idx].shapes[i].material.diffuseTexture);
		//glBindTexture(GL_TEXTURE_2D, models[cur_idx].shapes[i].material.diffuseTexture+);
		//Vector2 v = Vector2(0.5f, 0.5f);
		//glUniform2fv(iLocTex,, v);
		//cout << models[cur_idx].shapes[i].material.offsets.size() << endl;
		glUniform1i(iLoctexcontrol, models[cur_idx].shapes[i].material.isEye);
		if (models[cur_idx].shapes[i].material.isEye == 1) {
			int cur_tex = models[cur_idx].cur_eye_offset_idx;
			//cur_tex = 3;
			Vector2 v = Vector2(models[cur_idx].shapes[i].material.offsets[cur_tex].x, models[cur_idx].shapes[i].material.offsets[cur_tex].y);
			glUniform2fv(iLoctexoffset, 1, &v[0]);
		}
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, cur_mag);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, cur_min);
		
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, cur_coord);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, cur_coord);
		//glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,  models[cur_idx].shapes[i].material.isEye+0.5);
		//glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,  models[cur_idx].shapes[i].material.isEye + 0.5);
		//if (i == 0)
		//cout << cur_coord << endl;
		//cout << models[cur_idx].shapes[i].material.diffuseTexture << endl;
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_U:
			...
			break;
		case GLFW_KEY_I:
			cout << "Translation Matrix = " << translate(models[cur_idx].position) << endl;
			cout << "Rotation Matrix = " << rotate(models[cur_idx].rotation) << endl;
			cout << "Scaling Matrix = " << scaling(models[cur_idx].scale) << endl;
		...
		case 'J':
			...
			break;
		case 'G':
			cur_mag = (cur_mag == GL_NEAREST) ? GL_LINEAR : GL_NEAREST;
			break;
		case 'B':
			cur_min = (cur_min == GL_NEAREST) ? GL_LINEAR_MIPMAP_LINEAR : GL_NEAREST;
			break;
		case GLFW_KEY_LEFT:
			...
			break;
		case GLFW_KEY_RIGHT:
			models[cur_idx].cur_eye_offset_idx = (models[cur_idx].cur_eye_offset_idx < models[cur_idx].max_eye_offset) ?  models[cur_idx].cur_eye_offset_idx + 1:0;
			break;
		default:
			break;
		}
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case Light_editing:
		if (cur_light_mode == spot) {
			main_light->cutoff = (main_light->cutoff + yoffset < 180) ? ((main_light->cutoff + yoffset > 0) ? main_light->cutoff + yoffset : 0) : 0;
		}
		else {
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	double x_diff = (xpos - x) / WINDOW_WIDTH;
	double y_diff = -1 * (ypos - y) / WINDOW_HEIGHT;
	// double max_diff = (abs(x_diff) > abs(y_diff)) ? x_diff : -y_diff;
	if (mouse_pressed) {
		switch (cur_trans_mode) {
		case GeoTranslation:
			models[cur_idx].position.x += x_diff;
			models[cur_idx].position.y += y_diff;
			break;
		case GeoRotation:
			models[cur_idx].rotation.y += x_diff;
			models[cur_idx].rotation.x -= y_diff;
			break;
		case GeoScaling:
			models[cur_idx].scale.x += x_diff;
			models[cur_idx].scale.y -= y_diff;
			break;
		case ViewCenter:
			main_camera.center.x += x_diff;
			main_camera.center.y += y_diff;
			setViewingMatrix();
			break;
		case ViewEye:
			main_camera.position.x -= x_diff;
			main_camera.position.y -= y_diff;
			setViewingMatrix();
			break;
		case ViewUp:
			main_camera.up_vector.x += x_diff;
			main_camera.up_vector.y += y_diff;
			setViewingMatrix();
			break;
		case Light_editing:
			main_light->position.x += x_diff;
			main_light->position.y += y_diff;
			break;
		}
	}
	x = xpos;
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case GeoRotation:
				...
				break;
			case Light_editing:
				main_light->position.x -= diff_x * 0.001;
				main_light->position.y += diff_y * 0.001;
				break;
			}
		}
	}
...
void setShaders()
{
	...
	iLocLights.position = glGetUniformLocation(p, "l_position");
	iLocLights.direction = glGetUniformLocation(p, "l_direction");
	...
	iLoctexoffset = glGetUniformLocation(p, "texoffset");
	iLoctexcontrol = glGetUniformLocation(p, "texcontrol");
...
vector<Shape> SplitShapeByMaterial(vector<GLfloat>& vertices, vector<GLfloat>& colors, vector<GLfloat>& normals, vector<GLfloat>& textureCoords, vector<int>& material_id, vector<PhongMaterial>& materials)
{
	...
	for (int m = 0; m < materials.size(); m++)
	{
		...
		if (!m_vertices.empty())
		{
			...
			//cout << tmp_shape.p_texCoord << endl;
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		if (string(materials[i].diffuse_texname).find("Eye") != std::string::npos) {
			material.isEye = 1;
			Offset(0.0f, 0.0f);
			material.offsets.push_back(Offset(0.0f, 0.0f));
			material.offsets.push_back(Offset(0.5f, 0.0f));
			material.offsets.push_back(Offset(0.0f, 0.25f));
			material.offsets.push_back(Offset(0.5f, 0.25f));
			material.offsets.push_back(Offset(0.0f, 0.5f));
			material.offsets.push_back(Offset(0.5f, 0.5f));
			material.offsets.push_back(Offset(0.0f, 0.75f));
			material.offsets.push_back(Offset(0.5f, 0.75f));
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109062531 HW3", NULL, NULL);
    if (window == NULL)
    {
