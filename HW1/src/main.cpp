...
polygonMode cur_polygonMode = FILL;
...
void ChangeSize(GLFWwindow* window, int width, int height)
{
	...
	proj.aspect= (GLfloat)width / height;
...
void drawPlane()
{
	...
	vector<GLfloat> Pvertices;
	vector<GLfloat> Pcolors;

	for (int i = 0; i < 18; i++) {
		Pvertices.push_back(vertices[i]);
		Pcolors.push_back(colors[i]);
	}
	...
	glBufferData(GL_ARRAY_BUFFER, Pvertices.size() * sizeof(GL_FLOAT), &Pvertices.at(0), GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	quad.vertex_count = Pvertices.size() / 3;


	glGenBuffers(1, &quad.p_color);
	glBindBuffer(GL_ARRAY_BUFFER, quad.p_color);
	glBufferData(GL_ARRAY_BUFFER, Pcolors.size() * sizeof(GL_FLOAT), &Pcolors.at(0), GL_STATIC_DRAW);
...
void RenderScene(void) {	
	...
	glPolygonMode(GL_FRONT_AND_BACK, cur_polygonMode);
	glUniformMatrix4fv(iLocMVP, 1, GL_FALSE, mvp);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if(action == GLFW_PRESS){
		...
		{
		case 'W':
			if (cur_polygonMode == GL_LINE)
				cur_polygonMode=FILL;
			else if (cur_polygonMode == GL_FILL)
				cur_polygonMode=LINE;
			break;
		case 'Z':
			cur_idx--;
			if (cur_idx < 0)
...
{
	// [TODO] Call back function for mouse  GLFW_MOUSE_BUTTON_LEFT
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Student 109062531 HW1", NULL, NULL);
    if (window == NULL)
    {
